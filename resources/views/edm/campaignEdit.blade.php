...
@section('label', 'Update A New Edm Campaign')
@section('content')
	<style>
		table th{
			text-align:center;
	...
	<form  action="/edm/campaign/update" id="form" novalidate method="POST" onsubmit="return validate_form()" data-content="{{$data['content']}}">
		{{ csrf_field() }}
				...
				<div class="portlet light portlet-fit bordered ">
					...
					<div class="portlet-title">
						<div class="caption">
							...
							<span class="caption-subject bold font-green">Update A New Edm Campaign</span>
						</div>
					</div>
								...
								<div class="col-lg-8">
										...
										<div class="input-group ">
											...
											<select class="btn btn-default" id="template_id" data-width="100%" name="template_id" style="width:100%">
												...
												@foreach ($tmpData as $key => $value)
													<option value="{{$key}}" @if($data['template_id']==$key) selected  @endif>{{$value}}</option>
												@endforeach
											</select>
										</div>
									</div>
										...
										<div class="input-group ">
											...
											<select class="btn btn-default" id="marketplaceid" data-width="100%" name="marketplaceid" style="width:100%">
												@foreach ($site as $key => $value)
													<option value="{{ $value->marketplaceid }}" @if($data['marketplaceid']==$value->marketplaceid) selected  @endif>{{ $value->domain }}</option>
												@endforeach
											</select>
										</div>
									</div>
									<div class="form-group">
										...
										<div class="input-group ">
											...
											<input type="text" class="form-control" name="asin" id="asin" value="{{$data['asin']}}"  >
										</div>
									</div>
									<div class="form-group">
										<label>Subject</label>
										...
										<div class="input-group ">
											...
											<select class="mt-multiselect btn btn-default" id="tag_id" multiple="multiple" data-width="100%" data-action-onchange="true" name="tag_id[]" id="tag_id[]" >
												@foreach ($tagData as $key => $value)
														<option value="{{$key}}" {{ in_array($key,$data['tag_ids'])?'selected':''}}>{{$value}}</option>
												@endforeach
											</select>
										</div>
									</div>
										...
										<div class="col-md-4" style="margin-left:-16px;margin-bottom:10px;">
											...
											<div class="input-group ">
												...
												<select class="btn btn-default" id="set_sendtime" data-width="100%" data-action-onchange="true" name="set_sendtime" >
													<option value="0" @if($data['sendtime_type']==0) selected @endif>immediately</option>
													<option value="1" @if($data['sendtime_type']==1) selected @endif>date time</option>
												</select>
											</div>
										</div>
										<div class="col-md-4 senddate" style="margin-top:22px;display:none;">
											<input class="form-control" value="{{$data['set_sendtime']}}" id="senddate" name="senddate"/>
										</div>
									</div>
								</div>
								<div class="col-lg-12">
	...
	<script>
        ...
        ue.ready(function() {
            ...
            ue.setContent($('#form').attr('data-content'));
        });
