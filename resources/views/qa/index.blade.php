...
<html lang="en">
    ...
    height: 55px;
        ...
        .ztree li a {width:100%;height:40px;padding-top: 0px; border-bottom: 1px solid #f3f4f6;}
        .ztree li a:hover {text-decoration:none; background-color: #E7E7E7;}
        .ztree li a span.button.switch {visibility:hidden}
        .ztree.showIcon li a span.button.switch {visibility:visible}
        .ztree li a.curSelectedNode {background-color:#ffffff;border:0;height:40px;}
	...
	<div class="search-bar" style="  position: fixed; z-index:999;background: #eee;">
                                <div>
								<form method="get">
                                    <input type="hidden" name="knowledge_type" value="{{$knowledge_type}}" />
                                        ...
                                        <div class="col-md-4" style=" padding-left: 0px;">
                                            <div class="input-group">
                                                ...
                                                @foreach($groups as $key=>$val)
                                                    <option value="{{$val}}" <?php if($val==$group) echo 'selected';?>>{{$val}}</option>
                                                @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-4" style=" padding-left: 0px;">
                                            <div class="input-group">
                                                ...
                                                @foreach($for_product2 as $key=>$val)
                                                    <option value="{{$val}}" <?php if($val==$item_group) echo 'selected';?>>{{$val}}</option>
                                                @endforeach
                                                </select>
                                            </div>
                                        </div>
  ...
  text-align: center;">Questions And Solutions</p>
                                        ...
                                        <div class="blog-post-content">
                                            ...
                                            <div class="blog-post-foot">
                                                ...
                                                <div class="blog-post-meta" style="float:right;">
													<?php if($qa->confirm == 2){ ?>
													    <span class="label label-sm label-success">Invaild</span>
													<?php }else if($qa->confirm == 1){ ?>
    ...
    @endforeach
								...
								<div style="clear: both;"></div>
		<div style="text-align:center;">{!! $qas->appends(['keywords' => $keywords])->links() !!}</div>
		</div>
    </body>
...
<SCRIPT type="text/javascript">
    ...
    var zNodes =[
            ...
            ?>
        { "id":"<?=$val['id']?>", "pId":"<?=$val['category_pid']?>", "name":"<?=$val['category_name']?>","click":"s_url('<?=$val['category_name']?>');",open:true},
        <?php
        }
        ?>
...
//        location.href='/reviewexport?asin_status='+(($("select[name='asin_status[]']").val())?$("select[name='asin_status[]']").val():'')+'&keywords='+$("input[name='keywords']").val()+'&date_from='+$("input[name='date_from']").val()+'&date_to='+$("input[name='date_to']").val()+'&nextdate='+$("input[name='nextdate']").val()+'&follow_status='+(($("select[name='follow_status[]']").val())?$("select[name='follow_status[]']").val():'')+'&user_id='+(($("select[name='user_id[]']").val())?$("select[name='user_id[]']").val():'')+'&site='+(($("select[name='site[]']").val())?$("select[name='site[]']").val():'')+'&rating='+$("select[name='rating']").val()+'&bgbu='+$("select[name='bgbu']").val()+'&vp='+$('select[name="vp"]').val()+'&rc='+$('select[name="rc"]').val()+'&del='+$('select[name="del"]').val();
    ...
    function s_url(knowledge_type){
        location.href='/question?knowledge_type='+knowledge_type+'&group='+$("#group").val()+'&item_group='+$("#item_group").val()+'&keywords='+$("input[name='keywords']").val();
    }
