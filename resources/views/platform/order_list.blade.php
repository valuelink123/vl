...
<style type="text/css">
        ...
        <div class="col-md-12">
            ...
            <div class="portlet light bordered">
                <div class="table-toolbar">
                    <form role="form" action="{{url('platformorder')}}" method="GET">
                        {{ csrf_field() }}
                        <div class="row">
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
						<input type="text" class="form-control " name="reference_no" placeholder="平台订单号">
						</div>

                        <div class="col-md-2">
						<input type="text" class="form-control " name="order_code" placeholder="谷仓订单号">
						</div>
                        ...
                        <div class="col-md-2">
						<input type="text" class="form-control " name="email" placeholder="客户邮箱">
						</div>
						</div>
                        <div class="row" style="margin-top:20px;">
                        
						<div class="col-md-2">
						<select class="mt-multiselect btn btn-default " multiple="multiple" data-label="left" data-width="100%" data-filter="true" data-action-onchange="true" data-none-selected-text="同步操作" name="sync[]" id="sync[]">
                            @foreach (\App\Models\PlatformOrder::SYNC as $k=>$v)
                                <option value="{{$k}}">{{$v}}</option>
                            @endforeach
                        </select>
						</div>
                        ...
                        <div class="col-md-2">
						<select class="mt-multiselect btn btn-default " multiple="multiple" data-label="left" data-width="100%" data-filter="true" data-action-onchange="true" data-none-selected-text="同步状态" name="sync_status[]" id="sync_status[]">
                            @foreach (\App\Models\PlatformOrder::SYNC_STATUS as $k=>$v)
                                <option value="{{$k}}">{{$v}}</option>
                            @endforeach
                        </select>
						</div>
                ...
                <div class="portlet-title">
                    <div class="caption font-dark">
                        ...
                        <span class="caption-subject bold uppercase">平台订单表</span>
                        <a data-target="#ajax" data-toggle="modal" href="/platformorder/create"> 
                    ...
                    <div class="btn-group " style="float:right;margin-top:10px;">
                        <div class="table-actions-wrapper" id="table-actions-wrapper">
                            <select id="confirmStatus" class="table-group-action-input form-control input-inline">
                                ...
                                <option value="0">同步状态->等待同步</option>
                                <option value="-1">同步操作->取消订单</option>
                                <option value="-2">同步状态->取消同步</option>
                            </select>
    ...
    <script>
        ...
        var initTable = function () {
            ...
            grid.setAjaxParam("order_code", $("input[name='order_code']").val());
            grid.setAjaxParam("country_code", $("input[name='country_code']").val());
            ...
            grid.setAjaxParam("sync", $("select[name='sync[]']").val());
            grid.setAjaxParam("sync_status", $("select[name='sync_status[]']").val());
            grid.init({
                src: $("#datatable_ajax"),
            ...
            $(".btn-group").unbind("click").on('click', '.table-status-action-submit', function (e) {
                ...
                if (confirmStatus.val() != "" && grid.getSelectedRowsCount() > 0) {
                    ...
                    $.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "{{ url('platformorder/batchUpdate') }}",
...
$(function() {
	...
	$('#datatable_ajax').on('click', 'td:not(:has(input))', function (e) {
        ...
        $('#ajax').modal({
            remote: '/platformorder/'+recordId+'/edit'
        });
    } );
