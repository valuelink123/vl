                ...
                <form id="update_form"  name="update_form" >
                    ...
                    <div class="form-body">
						...
						<div class="form-group col-md-12">
                            <label>平台发货方式 * </label>
                            <input type="text" class="form-control" name="platform_shipping_method" id="platform_shipping_method" value="{{array_get($form,'platform_shipping_method')}}" >  
                        </div>

                        <div class="form-group col-md-12">
                            <label>谷仓仓库发货方式 *</label>
                            
                            <select class="mt-multiselect form-control " name="shipping_method" id="shipping_method" data-label="left" data-width="100%" data-filter="true" data-action-onchange="true">
							<?php 
							foreach(\App\Models\GuCangShippingMethod::all() as $k=>$v){ 	
								echo '<option value="'.$v->warehouse_code.'|'.$v->code.'" '.(($v->warehouse_code.'|'.$v->code==array_get($form,'warehouse_code').'|'.array_get($form,'shipping_method'))?'selected':'').'>'.$v->warehouse_code.' - '.$v->name.' - '.$v->code.'</option>';
							}?>
							</select>
                        </div>
