...
@section('crumb')
    @include('layouts.crumb', ['crumbs'=>['CashBack']])
@endsection
@section('content')
...
th,td,td>span {
    ...
    <script>
        ...
        $theTable.dataTable({
            ...
            columns: [
                ...
                {data: 'processor', name: 'processor', width: "120px"},
                {
                    ...
                    orderable: false,
                    render(data, type, row) {
                        return `<a class="btn btn-danger btn-xs" href="/cb/list/process?order_id=${data}&created_at=${encodeURIComponent(row.created_at)}" target="_blank">Process</a>`
                    }
                }
            ],
            ajax: {
        ...
        $(assignto).change(e => {
            ...
            postByJson('/cb/batchassigntask', {processor, ctgRows}).then(arr => {
