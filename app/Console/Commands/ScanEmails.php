...
class ScanEmails extends Command
{
     ...
     * @var string
     */
    protected $signature = 'scan:email {Id} {--time=}';
    ...
    public function saveEmails($lastMailDate){
		...
		$oClient = new Client([
			'host'          => $this->runAccount['imap_host'],
			'port'          => $this->runAccount['imap_port'],
			'encryption'    => $this->runAccount['imap_ssl'],			
			'validate_cert' => true,
			'username'      => $this->runAccount['account_email'],
			'password'      => $this->runAccount['password'],
			'protocol'      => 'imap'
		]);
		$oClient=$oClient->connect();
		$aFolders = $oClient->getFolders(false);
		foreach($aFolders as $oFolder){
			
			
			if ($oFolder->no_select) {
				continue;
			}
			if($oFolder->name=='Sent Messages' || $oFolder->name=='Deleted Messages' || $oFolder->name=='Drafts'){
				continue;
			}
			if($oFolder->name=='Outbox' || $oFolder->name=='Sent' || $oFolder->name=='Deleted' || $oFolder->name=='Drafts'){
				continue;
			}
			
			$messages = $oFolder->query()->since(date('d.m.Y',strtotime($lastMailDate)-1800))->limit(10, 2)->get();
			print_r($messages);
			die();
