...
class SkuController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		b.item_name
from (select asin,site,max(item_no) as item_code,max(item_status) as status,max(status) as pro_status, max(bg) as bg,max(bu) as bu,max(sap_seller_id) as sap_seller_id from asin where status in ('S','A') group by asin,site) as a
