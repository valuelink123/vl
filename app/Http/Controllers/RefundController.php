...
//		$data['fromDate'] = '2021-01-15';//测试日期
	...
	public function getSql($search)
	{
		...
		if(isset($search['account']) && $search['account']){
			$where.= " and finances_refund_events.seller_account_id in (".$search['account'].")";
		}
		...
		$refund_where = $where;//refund_where可搜索时间，账号，order id，currency
		...
		$sql = "select SQL_CALC_FOUND_ROWS any_value(orders.id) as id,finances_refund_events.seller_account_id,finances_refund_events.amazon_order_id,any_value(posted_date) as posted_date,any_value(currency) as currency,sum(finances_refund_events.amount) as refund_amount,any_value(settlement_id) as settlement_id,any_value(settlement_date) as settlement_date,any_value(orders.asins) as asins,refund_commission  
				from finances_refund_events 
				left join orders on orders.seller_account_id = finances_refund_events.seller_account_id and orders.amazon_order_id = finances_refund_events.amazon_order_id 
				left join (
					select sum(amount) as refund_commission,seller_account_id,amazon_order_id  from finances_refund_events {$refund_where} 
					and type = 'RefundCommission' 
					group by seller_account_id,amazon_order_id
				) as refund_commission on refund_commission.seller_account_id = finances_refund_events.seller_account_id and refund_commission.amazon_order_id = finances_refund_events.amazon_order_id
				left join (
				...
				) as settlement on finances_refund_events.seller_account_id=settlement.seller_account_id and  finances_refund_events.amazon_order_id=settlement.order_id 
				{$where} and type != 'RefundCommission' 
				group by finances_refund_events.seller_account_id,finances_refund_events.amazon_order_id 
