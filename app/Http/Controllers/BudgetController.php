...
right join budget_skus as c on b.sku=c.sku and b.site=c.site where ((a.month>='".$year_from_arr[0]."01' and a.month<='".$year_from_arr[0]."12' ) or a.month is null) $where order by b.id,a.month asc");
    ...
    {	
		...
		$data['base_data']['cold_storagefee']=round(array_get($storage_fee,'2_10_fee',0)*$data['base_data']['then_volume']/1000000/(($year==2020 && $quarter==1)?8:6),4);
		$data['base_data']['hot_storagefee']=round(array_get($storage_fee,'11_1_fee',0)*$data['base_data']['then_volume']/1000000/(($year==2020 && $quarter==1)?8:6),4);
