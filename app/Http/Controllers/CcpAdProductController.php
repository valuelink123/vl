...
class CcpAdProductController extends Controller
{
	...
	* ���ͳ��������
	...
	public function showTotal()
	{
		...
		$site = isset($search['site']) ? $search['site'] : '';//վ�㣬Ϊmarketplaceid
		$account = isset($search['account']) ? $search['account'] : '';//�˺�id,seller_id
		...
		//�û�Ȩ�����ݣ�ͨ��sap_asin_match_sku��õ��ɲ鷶Χ��asin_selersku������ݣ�
	...
	public function list(Request $req)
	{
		...
		$data = $this->getDealData($_data,$site);//�õ�����������

		return compact('data', 'recordsTotal', 'recordsFiltered');
	}
	/*
	 * �û�Ȩ�����ݣ�ͨ��sap_asin_match_sku��õ��ɲ鷶Χ��asin_selersku�������
	 */
	...
	{
		$site = isset($search['site']) ? $search['site'] : '';//վ�㣬Ϊmarketplaceid
		$account = isset($search['account']) ? $search['account'] : '';//�˺�id,����115,137
		$bg = isset($search['bg']) ? $search['bg'] : '';
		$bu = isset($search['bu']) ? $search['bu'] : '';
		$asin = isset($search['asin']) ? trim($search['asin'],'+') : '';//asin������ֵ
		$this->start_date = isset($search['start_date']) ? $search['start_date'] : '';
		$this->end_date = isset($search['end_date']) ? $search['end_date'] : '';
		$type = isset($search['type']) ? $search['type'] : '';
	...
	public function export(Request $req)
	{
		...
		$arrayData[] = $headArray;
		$site = isset($req['site']) ? $req['site'] : '';//վ�㣬Ϊmarketplaceid
		$data = $this->getDealData($_data,$site);//�õ�����������
		...
		if($arrayData){
			...
			header('Content-Disposition: attachment;filename="CCP_AdProduct.xlsx"');//���������������������
