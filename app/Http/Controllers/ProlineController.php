    ...
    public function get(Request $request)
    {
		...
		foreach ( $lists as $list){
			...
			array(
				...
				$list['sku_price'],
				($list['ZCRATIO']*100).'%',
				round($list['sku_price']*$list['ZCRATIO'],2),
				$list['NETPR2'],
				$list['Z003'],
				$list['NETPR'],
				$list['CCF1'],
				...
				$list['FBAPRICE'],
				$list['VERPR_FBA'],
				$list['FBAPRICE']+round($list['sku_price']*$list['ZCRATIO'],2)+(($list['RATE'])?round($list['VERPR_FBA']/$list['RATE']):$list['VERPR_FBA']),
				round($list['sku_price']-($list['FBAPRICE']+round($list['sku_price']*$list['ZCRATIO'],2)+(($list['RATE'])?round($list['VERPR_FBA']/$list['RATE']):$list['VERPR_FBA'])),2),
				round(($list['sku_price'])?(($list['sku_price']-($list['FBAPRICE']+round($list['sku_price']*$list['ZCRATIO'],2)+(($list['RATE'])?round($list['VERPR_FBA']/$list['RATE']):$list['VERPR_FBA'])))/$list['sku_price']):0,2),
				$list['YCL'],
				round((($list['sku_price'])?(($list['sku_price']-($list['FBAPRICE']+round($list['sku_price']*$list['ZCRATIO'],2)+(($list['RATE'])?round($list['VERPR_FBA']/$list['RATE']):$list['VERPR_FBA'])))/$list['sku_price']):0)-round($list['YCL'],4),2),
				...
				$list['FBM_SC'],
				$list['FBM_WJ'],
				$list['fba_stock']+$list['LBKUM_FBM']+$list['FBM_ZT']+$list['FBM_SC']+$list['FBM_WJ'],
